...
std:: vector <int> medianFilter:: getFilteredArray ( std:: string inputfile) {
	...
	{	//dealing with outermost pixels 
		...
		else 
		{
			...
			int filter[9] ; 
			filter[0]=pixelArray[i-101];
			filter[1]=pixelArray[i-100];
			filter[2]=pixelArray[i-99];
			filter[3]=pixelArray[i-1];
			filter[4]=pixelArray[i];
			filter[5]=pixelArray[i+1];
			filter[6]=pixelArray[i+99];
			filter[7]=pixelArray[i+100];
			filter[8]=pixelArray[i+101];
